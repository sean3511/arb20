...
$(".Alert_comming_soon").on("click", function () {
	// 显示具有 classB 的元素
	$(".Not_Open_now_btn").show();
    $(".Not_Open_now_btn").fadeIn(1000).delay(1000).fadeOut(1000);
});
...
window.onload = function () {
    ...
    // 等待 0.5 秒，然后开始倒计时
    setTimeout(() => {
        countdownElements[currentIndex].style.opacity = 1; // 初始显示数字 5
        countdownInterval = setInterval(showNextCountdown, 500);
    }, 500);

    function showNextCountdown() {
        countdownElements[currentIndex].style.opacity = 0; // 隐藏当前元素
        currentIndex = (currentIndex + 1) % countdownElements.length; // 更新索引
        countdownElements[currentIndex].style.opacity = 1; // 显示下一个元素

        // 如果是最后一个数字，设置延迟定时器，在倒计时结束后关闭整个母div，并还原页面滚动
        if (currentIndex === countdownElements.length - 1) {
            clearInterval(countdownInterval); // 清除倒计时定时器
            setTimeout(() => {
                countdownContainer.style.display = 'none'; // 隐藏整个母div
                document.body.style.overflow = 'visible'; // 还原页面滚动
            }, 1000);
        }
    }
};
